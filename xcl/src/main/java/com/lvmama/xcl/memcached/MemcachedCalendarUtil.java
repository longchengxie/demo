...
public class MemcachedCalendarUtil {
	...
	private final static String CALENDAR_CACHE_SERVER = "calendar.cache.server";
	...
	private void initCalendarMemCached() {
		try {
			String serverStr = getProperty(CALENDAR_CACHE_SERVER);
			if (StringUtils.isBlank(serverStr)) {
				log.warn("sweet config load calendar memcached is failed");
				isSweet = false;
				...
				serverStr = getProperty(CALENDAR_CACHE_SERVER);
			}
			//数据缓存服务器，“,”表示配置多个memcached服务
			String[] calendarServers = serverStr.replaceAll(" ", "").split(",");

			SockIOPool sessionPool = SockIOPool.getInstance("calendarCacheServer");

			sessionPool.setServers(calendarServers);
